...
let string_of_result (n : int option) = match n with 
  | None -> "Illegal Expression"
  | Some n -> string_of_int n
    
let eval1 (v1 : int option) (v2 : int option) (op : int -> int -> int) : int option = match v1, v2 with
  | Some n1, Some n2 -> Some (op n1 n2)
  | None, _ | _, None -> None
    
let rec eval (ast : ast) : int option = match ast with
    Const(n) -> n
